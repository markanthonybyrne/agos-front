...
# EmpireQuest Frontend - Production Deployment Script
# This script builds the frontend with production config and deploys to the server
#
# Usage:
#   ./deploy-production.sh                    # Fast build (no image compression)
#   ./deploy-production.sh --compress         # Build with image compression (slower)
#   ./deploy-production.sh --fast             # Fast build (explicit, default)
...
if [[ "$1" == "--compress" ]] || [[ "$1" == "-c" ]]; then
    ...
    echo -e "${YELLOW}Usage: ./deploy-production.sh [--compress|--fast]${NC}"
...
echo -e "${BLUE}════════════════════════════════════════${NC}"
echo -e "${GREEN}🚀 EmpireQuest Frontend - Production Deployment${NC}"
echo -e "${BLUE}════════════════════════════════════════${NC}"
echo ""
...
if [ "$ENABLE_COMPRESSION" = true ]; then
    echo -e "${GREEN}📦 Building frontend with production configuration (with image compression)...${NC}"
    ...
    BUILD_CMD_SKIP="npm run build:production:skip-check"
else
    echo -e "${GREEN}📦 Building frontend with production configuration (fast build, no compression)...${NC}"
    export SKIP_IMAGE_COMPRESSION=true
    BUILD_CMD="npm run build:fast:production"
    BUILD_CMD_SKIP="npm run build:fast:production"
fi
